...
def manejar_cliente(conn, addr, CONNECTION):
...
def main():
    ...
    server_socket.listen(300)  # Escuchar 300 conexiones

    with ssl_context.wrap_socket(server_socket, server_side=True) as s:
        while True:
            conn, addr = s.accept()
            thread = threading.Thread(target=manejar_cliente, args=(conn, addr, CONNECTION))
            thread.start()
